...
public:
    ...
    //   背景：本类持有 fd/m_ptr/drm_fd 等裸资源，默认浅拷贝会使两个
    //   对象析构时各自 release() → 同一 fd 被 close 两次、同一区间被
    //   munmap 两次，第二次 close 的 fd 号可能已被其他线程复用，
    //   造成误关无关句柄。需要共享时用 std::shared_ptr<DmaFrameBuffer>
    //   （项目已注册该元类型）或显式 dup(fd)。
