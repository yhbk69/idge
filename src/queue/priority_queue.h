...
//
// ⚠⚠【核心语义警示（上轮审查确认）：tryPop/waitAndPop 名为 Pop 实为 Top】
//   二者只读取堆顶 c_.front() 拷贝给调用方，不移除元素、不改堆结构。
//   隐患：
//   1) 消费方循环调用 waitAndPop 会反复拿到同一最高优先级元素——
//      同一条检测结果被处理多次（重复告警/重复存图），且队列永远
//      "非空"，等待新数据的逻辑失效（忙等自旋风险）；
//   2) 想真正消费必须额外手动调用 pop()，二者不成对即产生
//      "读了没删"（重复消费）或"删了没读"（丢数据）的错配；
//   3) 与标准库 priority_queue::pop()（只删不读）语义相反，
//      极易被熟悉 STL 的维护者误用。
//   若需"取出即删除"应使用 top()+pop() 组合或改用其他队列实现。
//
...
class PriorityQueue
{
  public:
    ...
    //
    // 注意：这个方法不会移除元素，只是读取（语义等同 std 的 top()，
    //   见文件头"名为 Pop 实为 Top"警示——重复调用返回同一元素）
    // ============================================================================
