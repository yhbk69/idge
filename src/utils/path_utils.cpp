 ...
 * @brief 路径工具函数实现
 *
 * 作用：实现文件路径解析相关的工具函数，包括环境变量读取、
 *       可执行文件目录获取、搜索根目录构建、以及多候选路径解析。
 */
...
// ============================================================================
// 匿名命名空间 —— 内部辅助函数（仅本文件可见，避免链接冲突）
// ============================================================================
namespace
{

// ============================================================================
// append_root_if_unique —— 向根目录列表追加唯一路径
// ============================================================================
// 作用：将规范化后的路径加入根目录列表，如果该路径尚未存在则添加，避免重复。
//       规范化包括：解析符号链接、消除 . 和 ..、词法规范化。
// @param roots 根目录列表（输出参数）
// @param root 待添加的路径
// ============================================================================
...
// ============================================================================
// 作用：安全地读取环境变量。传入空指针或变量不存在时均返回空字符串。
// ============================================================================
...
// ============================================================================
// 作用：通过 Linux /proc/self/exe 符号链接读取当前可执行文件的绝对路径，
//       然后提取父目录。如果读取失败，回退到当前工作目录。
// ============================================================================
...
// ============================================================================
// default_search_roots —— 返回默认的搜索根目录列表
// ============================================================================
// 作用：构建按优先级排序的搜索根目录列表，用于查找配置文件或资源文件。
//       优先级从高到低：
//       1. 环境变量 MYDEMO_ASSET_ROOT
//       2. 当前工作目录及其上级目录
//       3. 可执行文件所在目录及其上级目录
// ============================================================================
...
// ============================================================================
// resolve_existing_path —— 在多个候选路径中解析存在的绝对路径
// ============================================================================
// 作用：搜索策略：
//       1. 如果提供了环境变量名，优先检查环境变量指定的路径是否存在
//       2. 遍历每个候选路径，如果是绝对路径且存在则直接返回
//       3. 否则在默认搜索根目录下拼接候选路径，找到第一个存在的文件
//       4. 所有候选都不存在时，回退返回第一个搜索根 + 第一个候选路径
// ============================================================================
                                  ...
                                  const char *env_var)
{
    ...
    // 如果所有候选路径都不存在，则回退：使用第一个搜索根目录 + 第一个候选路径
    // 并返回词法规范化后的路径（不检查存在性）
...
// ============================================================================
// resolve_config_path —— 解析配置文件路径（简化版本）
// ============================================================================
// 作用：将单个文件名放入候选列表，调用通用解析函数 resolve_existing_path。
// ============================================================================
