...
static int convert_image_rga(image_buffer_t* src_img, image_buffer_t* dst_img, image_rect_t* src_box, image_rect_t* dst_box, char color)
{
    ...
    if (drect.width != dstWidth || drect.height != dstHeight) {
        ...
        // bug: address cannot above 4g, so use memset, time is about 0.15ms;
        ret_rga = imfill(rga_buf_dst, dst_whole_rect, imcolor); 
        // ret_rga = IM_STATUS_FAILED;
        if (ret_rga <= 0) {
            if (dst != NULL) {
                ...
                //printf("%s use: %f ms\n", "memset time", ((stop_time.tv_sec*1000000 + stop_time.tv_usec) - (start_time.tv_sec*1000000 + start_time.tv_usec))/1000.0);
            } else {
    ...
    struct timeval start_time, stop_time;
    gettimeofday(&start_time, NULL);
    //TODO:improcess时间20ms,需要优化
    ret_rga = improcess(rga_buf_src, rga_buf_dst, pat, srect, drect, prect, usage);
    gettimeofday(&stop_time, NULL);
    //printf("%s use: %f ms\n", "rga improcess time", ((stop_time.tv_sec*1000000 + stop_time.tv_usec) - (start_time.tv_sec*1000000 + start_time.tv_usec))/1000.0);
    if (ret_rga <= 0) {
