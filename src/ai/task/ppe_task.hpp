...
public:
    ...
    // 流程：
    //   1. 停止并确认旧推理线程已退出（有界等待）
    //   2. 创建新模型（失败则原样重启旧模型线程）
    //   3. 替换模型 + 复位队列（reopen）+ 重启推理线程
    // 参数：
    //   newPath - 新模型文件路径
    //   newLabelPath - 新标签文件路径
    //   coreMask - NPU 核心掩码
    // 返回：true=成功；false=失败（新模型加载失败=旧模型继续工作；
    //   线程未在时限内退出=本槽位推理已停止，可待 isThreadFinished 后重试）
    // =================================================================
    ...
    // =================================================================
    // isThreadFinished - 推理线程是否已确认退出
    // =================================================================
    // stopBestEffort 超时 detach 后，上层可轮询本接口，待返回 true 再销毁对象
    bool isThreadFinished() const { return finished_.load(); }

    // =================================================================
    // currentCoreMask - 当前生效的 NPU 核心掩码
    // =================================================================
    rknn_core_mask currentCoreMask() const { return config.core_mask; }
...
private:
    // =================================================================
    // restartThread - 复位队列并重启推理线程（内部辅助）
    // =================================================================
    // 前提：旧线程已确认退出（finished_），否则形成双消费者 + UAF。
    // requestStop() 会 close 队列且 close 原本单向，重启必须 clear+reopen。
    // =================================================================
    void restartThread();
